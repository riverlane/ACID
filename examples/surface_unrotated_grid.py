...
from acid.codes.surface.unrotated_grid_square_edges import build_unrotated_surface_grid_square_edges_code as build_surface_square_edges
...
def run(
    ...
    base, coords = build_surface_square_edges(int(distance))
    embedding = CoordMapEmbedding(coords)
    print(f"Built surface (square edges with boundary singles), d={distance}: n={base.num_qubits}, shapes={len(base.shapes)}")
    ...
    # Output
    if out is None:
        out_name = f"surface_square_edges_d{distance}_dropQ{n_dropped_qubits}_dropE{n_dropped_couplers}_L{L_found}_R{rounds}.stim"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Unrotated surface (grid): compile and emit memory experiment")
