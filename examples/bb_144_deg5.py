...
def run(
    ...
    key = "bb144"
    spec = get_spec(key)
    base, embedding, connections = deg5.build_code_from_spec(spec)
    print(f"Built code '{key}' (deg5): n={base.num_qubits}, shapes={len(base.shapes)}")
    ...
    def parse_qubits(s: str | None) -> List[int]:
        ...
        return sorted(list({int(p) for p in parts}))
    ...
    # Output
    if out is None:
        out_name = f"bb_144_deg5_dropQ{n_dropped_qubits}_dropE{n_dropped_couplers}_L{L_found}_R{rounds}.stim"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="BB bb144 (deg5): compile and emit memory experiment")
