...
def run(
    ...
    kept_schedules_per_quasi_stabilisers: int = 20,
    verbose: bool = False,
    no_web_prompt: bool = False,
    drop_qubits: str | None = None,
    drop_couplers: str | None = None,
) -> None:
    # Heads-up for users: this case can be heavy without pruning
    print("[warning] Square ancilla-free (deg4) colour can take a while. Pruning is enabled by default (M=20).", flush=True)
    if verbose:
        os.environ["FAB_SCHEDULE_ENUM_VERBOSE"] = "1"
        os.environ["FAB_SCHEDULE_BUILD_VERBOSE"] = "1"
    # Square-grid ancilla-free colour code via deg4 hex builder (extra rung per hex)
    base, coords = build_colour_hex_code(int(distance), deg4=True)
    embedding = CoordMapEmbedding(coords)
    print(f"Built colour code (square ancilla-free, deg4), d={distance}: n={base.num_qubits}, shapes={len(base.shapes)}")
    ...
    # Enable pruning by default for deg4 colour (default M=20)
    try:
    ...
    # Output
    if out is None:
        out_name = f"colour_square_deg4_d{distance}_dropQ{n_dropped_qubits}_dropE{n_dropped_couplers}_L{L_found}_R{rounds}.stim"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Colour code (square ancilla-free, deg4): compile and emit memory experiment")
    ...
    ap.add_argument("--kept-schedules-per-quasi-stabilisers", type=int, default=20, help="Pruning: keep at most M schedules per quasi-stabiliser (default: 20)")
    ...
    run(
        ...
        kept_schedules_per_quasi_stabilisers=args.kept_schedules_per_quasi_stabilisers,
