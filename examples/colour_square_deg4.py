...
def run(
    ...
    print(
        "[warning] Square ancilla-free (deg4) colour can take a while. Pruning is enabled by default (M=20).",
        flush=True,
    )
    if verbose:
    ...
    print(
        f"Built colour code (square ancilla-free, deg4), d={distance}: n={base.num_qubits}, shapes={len(base.shapes)}"
    )
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Colour code (square ancilla-free, deg4): compile and emit memory experiment"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=20,
        help="Pruning: keep at most M schedules per quasi-stabiliser (default: 20)",
    )
    ap.add_argument(
        "--verbose",
        action="store_true",
