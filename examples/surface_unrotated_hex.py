...
from acid.codes.surface.unrotated_hex import build_unrotated_surface_hex_code
...
def run(
    ...
    base, coords = build_unrotated_surface_hex_code(int(distance))
    embedding = CoordMapEmbedding(coords)
    print(f"Built unrotated surface (hex), d={distance}: n={base.num_qubits}, shapes={len(base.shapes)}")
    ...
    # Output
    if out is None:
        out_name = f"surface_hex_d{distance}_dropQ{n_dropped_qubits}_dropE{n_dropped_couplers}_L{L_found}_R{rounds}.stim"
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Unrotated surface (hex): compile and emit memory experiment")
