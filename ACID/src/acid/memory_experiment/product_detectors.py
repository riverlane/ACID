...
def plan_product_detectors(
    *,
    ...
    products = [p for p in dcode.products_list() if len(p.members) > 1 and ((p.pauli_type == 'X' and include_x) or (p.pauli_type == 'Z' and include_z))]
    if not products:
        return plan
    ...
    for p in products:
        ...
        # Build completion sequence: B0 (init), then for each round the sorted completions, then BN (final)
        periodic = sorted(prod_plan.completions.get(p.label, []))
        completions: List[Tuple[str, Optional[Tuple[int, int]], Dict[str, Tuple[int, int]], Optional[Tuple[int, int]]]] = []
        ...
        for r in range(1, R + 1):
            for t_star in periodic:
                ...
                for m in members:
                    ...
                    if not ts_le:
                        raise RuntimeError(f"Product completion invariant violated: member {m} not measured by t*={t_star} in round {r} for product {p.label}")
                    t_m = max(ts_le)
                    member_latest[m] = (r, t_m)
                # A_i: earliest among member_latest
                A_rt = min(member_latest.values(), key=lambda rt: _lin_idx(L, rt[0], rt[1]))
                completions.append(('contract', (r, t_star), member_latest, A_rt))
        completions.append(('final_mpp', None, {}, None))

        if debug:
            try:
                print(f"[product-debug] Product {p.label} basis={basis} members={len(members)}")
                for idx, (kind, B_rt, member_latest, A_rt) in enumerate(completions):
                    if kind == 'contract':
                        print(f"  - B[{idx}] kind=contract B_rt={B_rt} A_rt={A_rt} c_q={{" + ", ".join(f"{m}:{rt}" for m, rt in member_latest.items()) + "}}")
                    else:
        ...
        def active_support_at(r: int, t: int, cqi: Dict[str, Tuple[int, int]], cqi1: Dict[str, Tuple[int, int]]) -> Set[int]:
            idx = _lin_idx(L, r, t)
            acc: Set[int] = set()
            for m in members:
        ...
        for i in range(len(completions) - 1):
            ...
            # Determine window bounds in linear indices
            ...
            # c_q for this and next completion
            ...
            if debug:
                try:
                    print(f"    [window {i}] start={{kind:{kind_a},A_rt:{A_rt_a},B_rt:{B_rt_a}}} -> end={{kind:{kind_b},B_rt:{B_rt_b}}}")
                    if latest_a:
                        print("      c_q_i   = {" + ", ".join(f"{m}:{rt}" for m, rt in latest_a.items()) + "}")
                    if latest_b:
                        print("      c_q_ip1 = {" + ", ".join(f"{m}:{rt}" for m, rt in latest_b.items()) + "}")
                except Exception:
                    pass

            # (1) First detector only: include init MPP product
            if i == 0:
                for m in members:
                    rec0 = log.init_mpp[basis][m]
                    recs.append(int(rec0))

            # (2) Per-layer contributions for A_i < l ≤ B_{i+1}
            ...
            for idx in range(A_lin + 1, min(B_next_lin, R * L) + 1):
                ...
                # Fail fast if any opposite-basis root overlaps
                if any((q in root_other) for q in S):
                    bad = [q for q in S if q in root_other]
                    raise RuntimeError(f"Product {p.label} support overlaps opposite-basis roots at r={r_i}, t={t_i}, qubits={bad}")
                added = 0
                for q in S:
                    if q in root_same:
            ...
            # (3) Final window includes final product from final MPP
            if kind_b == 'final_mpp':
                for m in members:
            ...
            info = DetectorInfo(
                ...
                label=p.label,
                basis=None,
                start={'type': kind_a, 'round': A_rt_a[0] if A_rt_a else None, 'layer': A_rt_a[1] if A_rt_a else None},
                end={'type': kind_b, 'round': B_rt_b[0] if B_rt_b else None, 'layer': B_rt_b[1] if B_rt_b else None},
            ...
            if debug:
                try:
                    print(f"      layers (r,t): active_qubits -> added_recs")
...
def build_product_plan(dcode: DefectiveCode, layers: List[SyndromeExtractionLayer]) -> ProductPlan:
    ...
    prod_members: Dict[str, Set[str]] = {p.label: set(p.members) for p in prods if len(p.members) > 1}
