...
class TemplateFactory:
    ...
    @staticmethod
    def _key(pauli_type: str, n_qubits: int, connectivity_subgraph: nx.Graph, SEC_cycle_length: int) -> Tuple:
        edges = tuple(sorted((min(u, v), max(u, v))
                      for (u, v) in connectivity_subgraph.edges()))
        return (pauli_type, int(n_qubits), int(SEC_cycle_length), edges)

    def get_or_create(self, pauli_type: str, n_qubits: int,
                      connectivity_subgraph: nx.Graph, SEC_cycle_length: int,
                      name: str = "",
                      ...
                      layer_hint: int | None = None) -> StabiliserTemplate:
        k = self._key(pauli_type, n_qubits,
                      connectivity_subgraph, SEC_cycle_length)
        obj = self._cache.get(k)
        if obj is not None:
            return obj
        obj = StabiliserTemplate(
            pauli_type, n_qubits, connectivity_subgraph, SEC_cycle_length,
            name=name,
