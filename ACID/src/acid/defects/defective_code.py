...
from acid.gf2_utils import gf2_rank, gf2_nullspace, gf2_bidiagonalize, gf2_rref_colwise, gf2_rref_numpy, gf2_is_in_span
...
def build_quasi_stabilisers(base: BaseCode, dropped_qubits: Set[int], defective_edges: Set[Tuple[int,int]] | None = None) -> List[QuasiStabiliser]:
...
class DefectiveCode:
    ...
    def __init__(self, code: BaseCode, dropped_nodes: List[int] | Set[int] = (), dropped_edges: List[Tuple[int,int]] = (), *, verify: bool = True) -> None:
        self.base_code = code
        ...
        for u, v, data in self.base_code.connectivity_graph.edges(data=True):
            ...
            if (a, b) in dropped_norm:
                continue
            self.connectivity_graph.add_edge(u, v, **{k: v for k, v in data.items() if k != 'defective'})
        ...
        self.all_quasis: List[QuasiStabiliser] = build_quasi_stabilisers(self.base_code, self.dropped_nodes, defective_edges=def_edges_norm)
        ...
        self.gauges: List[Tuple[QuasiProduct, QuasiProduct]] = self._build_gauges()
    ...
    def midcycle_untouched_stabilisers(self) -> CommutingPauliBasis:
        ...
        for label in self.quasi_labels:
            ...
            if stab.parent.pauli_type == 'X':
                x_supps.append(supp)
            else:
                z_supps.append(supp)
        return CommutingPauliBasis.from_supports(name="MidCycle/Base", priority=10, x_supports=x_supps, z_supports=z_supps, n=self.base_code.num_qubits)
    ...
    def midcycle_product_stabilisers(self) -> CommutingPauliBasis | None:
        ...
        if not x_supps and not z_supps:
            return None
        return CommutingPauliBasis.from_supports(name="Products", priority=0, x_supports=x_supps, z_supports=z_supps, n=self.base_code.num_qubits)
    ...
    def _gauge_pairs_mid_paulis(self) -> Tuple[List[PauliString], List[PauliString]]:
        ...
        Gx_ps = [PauliString.from_supports([i for i, b in enumerate(row) if b & 1], [], n) for row in self.GX]
        Gz_ps = [PauliString.from_supports([], [i for i, b in enumerate(row) if b & 1], n) for row in self.GZ]
        return Gx_ps, Gz_ps
    ...
    def _reify_quasi_templates(self) -> List[Tuple[StabiliserTemplate, List[int], str]]:
        ...
        for i, q in enumerate(self.all_quasis):
            ...
            qubit_map = [shape.qubit_map[old] for old in sorted(old_to_new.keys(), key=lambda x: old_to_new[x])]
    ...
    def _build_dropped_quasis(self) -> Dict[str, QuasiStabiliser]:
        ...
        for qq in sorted(int(q) for q in self.dropped_nodes):
            ...
            qx = QuasiStabiliser(pauli_type='X', support=frozenset({qq}), parent=shx, component_index=0, label=shx.label)
            out[qx.label] = qx
            # Z one-hot
            shz = StabiliserShape('Z', G1, 1, [qq], f"DropZ({qq})")
            qz = QuasiStabiliser(pauli_type='Z', support=frozenset({qq}), parent=shz, component_index=0, label=shz.label)
            out[qz.label] = qz
        return out
    
    def _build_gauges(self) -> List[Tuple[QuasiProduct, QuasiProduct]]:
        gauges: List[Tuple[QuasiProduct, QuasiProduct]] = []
        # Z gauges are rows i < r from U (Z space)
        for i in range(self.r):
            coeffs = self.U[i]
            members = [self.z_anti_qs[k].label for k, bit in enumerate(coeffs) if bit & 1]
            if members:
                gauges.append((QuasiProduct(label=f"QgZ_{i}", pauli_type='Z', members=members), None))
        ...
        for i in range(self.r): 
            ...
            if members:
                if i < len(gauges) and gauges[i][1] is None:
                    gauges[i] = (gauges[i][0], QuasiProduct(label=f"QgX_{i}", pauli_type='X', members=members))
                else:
                    gauges.append((None, QuasiProduct(label=f"QgX_{i}", pauli_type='X', members=members)))
            ...
            for q in sorted(int(q) for q in self.dropped_nodes):
                if 0 <= q < n:
                    ...
                    if not gf2_is_in_span(ex, self.HX) and not gf2_is_in_span(ez, self.HZ):
                        ...
                        zg = QuasiProduct(label=f"QgZ_drop_{q}", pauli_type='Z', members=[f"DropZ({q})"])
                        xg = QuasiProduct(label=f"QgX_drop_{q}", pauli_type='X', members=[f"DropX({q})"])
                        gauges.append((zg, xg))
        return gauges


    def _compute_diag_from_anticomm(self) -> Dict:
        ...
        ZN = len(self.z_anti_qs); XN = len(self.x_anti_qs)
    ...
    def _compute_logical_rows(self) -> Tuple[List[List[int]], List[List[int]]]:
        ...
        2) RREF SX, SZ; form nullspaces NX = Null(SZ), NZ = Null(SX).
        3) CX = [SX; NZ], CZ = [SZ; NX]; RREF both; the first m rows are independent; the
        ...
        CX = HX + NZ
        CZ = HZ + NX
        CX_rref, _ = gf2_rref_colwise(CX, clear_upper_triangle=False)
        CZ_rref, _ = gf2_rref_colwise(CZ, clear_upper_triangle=False)
        ...
        _, U, _, V_T, _, r = gf2_rref_numpy(np.array(C))
        ...
        Xp = apply_transform(U, X_rows)
        Zp = apply_transform(V_T.T, Z_rows)
    ...
    def _verify_logicals_and_gauges(self) -> None:
        ...
        """
        n = self.base_code.num_qubits
        n_eff = n - len(self.dropped_nodes)
        ...
        def check_block_is_identity(mat: List[List[int]], r0: int, c0: int, sz: int, label: str) -> None:
            for i in range(sz):
                for j in range(sz):
                    exp = 1 if i == j else 0
                    if mat[r0 + i][c0 + j] != exp:
                        raise AssertionError(f"{label} block not identity at ({i},{j})")
        def check_block_is_zero(mat: List[List[int]], r0: int, c0: int, rsz: int, csz: int, label: str) -> None:
            for i in range(rsz):
                for j in range(csz):
                    if mat[r0 + i][c0 + j] != 0:
        ...
        if kx != k_expected or kz != k_expected:
            raise AssertionError(f"Unexpected logical count: kx={kx}, kz={kz}, expected={k_expected}")
    ...
    def stats(self) -> Dict:
        ...
        def _is_drop_g(lab: str | None) -> bool:
            return bool(lab) and ("_drop_" in lab or lab.startswith("QgX_drop_") or lab.startswith("QgZ_drop_"))
        ...
        for pair in gauges:
            try:
                ...
                zg, xg = None, None
            if (zg is not None and _is_drop_g(getattr(zg, 'label', None))) or (xg is not None and _is_drop_g(getattr(xg, 'label', None))):
    ...
    def quasi_support(self, label: str) -> Tuple[str, List[int]]:
        ...
        if q is None:
            ...
            for stab in self.base_code.stabilisers.values():
                if stab.label == label:
    ...
    def visualisation_stim(self, embedding: Embedding, *, colour_map: Dict[str, str] | None = None, include_reset: bool = False, debug: bool = False) -> str:
        ...
        if getattr(self.base_code, 'connection_classes', None):
            ...
            if colour_map is None:
                ...
                palette = ["#4361ee", "#2a9d8f", "#e76f51", "#f4a261", "#e9c46a", "#8a5cff"]
        ...
        if debug:
            print("[viz] qubits=", self.base_code.num_qubits, "dropped_nodes=", len(self.dropped_nodes), "dropped_edges=", len(self.dropped_edges))
        ...
        label_to_quasi: Dict[str, QuasiStabiliser] = {q.label: q for q in self.all_quasis}
        untouched: List[Tuple[str, List[int]]] = []
        for label in self.quasi_labels:
            if label in self.nontrivial_idx:
                continue
            q = label_to_quasi.get(label)
        ...
        anticomm_items: List[Tuple[str, List[int]]] = []
        for q in nx.get_node_attributes(self.anticomm_graph, 'quasi').values():
            anticomm_items.append((q.pauli_type, sorted(q.support)))
        if debug:
            print("[viz] anticomm quasis (nodes):", len(list(self.anticomm_graph.nodes())), "rendered:", len(anticomm_items))

        # Product stabilisers: XOR supports of member quasis
        product_items: List[Tuple[str, List[int]]] = []
        if debug:
        ...
        gauge_items: List[Tuple[str, List[int]]] = []
        ...
        if not is_torus:
            for i, ln in enumerate(lines):
                if ln.startswith("##! EMBEDDING TYPE=TORUS "):
                    lines[i] = f"##! EMBEDDING TYPE=PLANE LX={embedding.width} LY={embedding.height}"
                    break
        return "\n".join(lines) + "\n"

    def write_visualisation(self, out_path, embedding: Embedding, *, colour_map: Dict[str, str] | None = None, include_reset: bool = True) -> None:
        """
        Write a .stim visualisation of the current code highlighting dropped
        qubits and edges (wrapper over visualisation_stim).
        """
        text = self.visualisation_stim(embedding, colour_map=colour_map, include_reset=include_reset)
