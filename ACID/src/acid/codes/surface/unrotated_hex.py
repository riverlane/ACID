...
def build_unrotated_surface_hex_code(d: int) -> Tuple[BaseCode, Dict[Tuple[int,int], int]]:
    """
    ...
    for i in range(0, d):
        for j in range(0, d):
            if i == d-1 and j == d-1: continue
            x1,y1 = 2*i, 2*j
            x2,y2 = 2*i+1, 2*j+1
            if hasq(x1,y1) and hasq(x2,y2): G.add_edge(idq(x1,y1), idq(x2,y2))
    # E2 edges
    for i in range(0, d):
        for j in range(0, d):
            x1,y1 = 2*i+1, 2*j+1
        ...
        for j in range(0, d-1):
            ...
            for (x1, y1), (x2, y2) in zip(order[:1], order[1:]):
                if not G.has_edge(idq(x1, y1), idq(x2, y2)):
                    raise RuntimeError(f"Local stabiliser connectivity missing for X({x},{y}) between {(x1,y1)} and {(x2,y2)}")
            path = _path_template(k := len(order))
    ...
    for i in range(0, d-1):
        ...
        for j in range(0, d):
            ...
            for (x1, y1), (x2, y2) in zip(order[1:], order[:1]):
                if not G.has_edge(idq(x1, y1), idq(x2, y2)):
                    raise RuntimeError(f"Local stabiliser connectivity missing for Z({x},{y}) between {(x1,y1)} and {(x2,y2)}")
            path = _path_template(k := len(order))
            qmap = [idq(px, py) for (px, py) in order]
