...
def build_toric_code(
    ...
    for ax in range(l):
        for ay in range(m):
            ...
            #   2: L(a2 q)
            #   3: R(q)
            L_q = emb.qubit_id(*q.to_coordinate_LR_tuple("L"))
            R_b2q = emb.qubit_id(*((b2 * q).to_coordinate_LR_tuple("R")))
            L_a2q = emb.qubit_id(*((a2 * q).to_coordinate_LR_tuple("L")))
            R_q = emb.qubit_id(*q.to_coordinate_LR_tuple("R"))
            ...
            #   2: R(q)
            #   3: L(b2^-1 q) = L(x q)
            R_a2invq = emb.qubit_id(*((a2_inv * q).to_coordinate_LR_tuple("R")))
            L_b2invq = emb.qubit_id(*((b2_inv * q).to_coordinate_LR_tuple("L")))
            ...
            if connectivity.lower() in ("grid", "grid4"):
                ...
                R_a2inv_b2_q = emb.qubit_id(*((a2_inv * b2 * q).to_coordinate_LR_tuple("R")))
