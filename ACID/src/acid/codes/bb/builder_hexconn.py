...
CODE_TABLE: dict[str, dict] = {
    ...
    'bb288': {
        ...
        'A_str': 'x^3 + y^7 + y^2',
        'B_str': 'y^3 + x + x^2',
        'a2': (0, 5), 'a3': (3, 10), 'b2': (11, 0), 'b3': (10, 3),
        'fx': 1, 'fy': 1,
    },
}
...
def build_code_from_spec(spec: CodeSpec, *, fx: Optional[int] = None, fy: Optional[int] = None) -> Tuple[BaseCode, SquareGridEmbedding, List[Tuple[int,int,str]]]:
    l, m = spec.l, spec.m
    ...
    if use_fx is None or use_fy is None:
        raise ValueError(f"Must specify homomorphism fx/fy for code {spec.key}; pass to build_code_from_spec or add to spec")
